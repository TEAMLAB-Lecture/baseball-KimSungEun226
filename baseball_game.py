...
def is_between_100_and_999(user_input_number):
    ...
    # ===Modify codes below=============
    # 조건에 따라 변환되어야 할 결과를 result 변수에 할당
    result = bool(int(user_input_number) >= 100 and int(user_input_number) < 1000)
...
def is_duplicated_number(three_digit):
    ...
    result = any([three_digit[0] == three_digit[1], three_digit[1] == three_digit[2], three_digit[0] == three_digit[2]])
    # ==================================
    return result
...
def get_not_duplicated_three_digit_number():
    ...
    while True:
        ...
        if not is_duplicated_number(str(temp)):
            break
    result = temp
    # ==================================
    return result
...
def is_yes(one_more_input):
    ...
    result = any([one_more_input.lower()== 'y', one_more_input.lower()== 'yes'])
    # ==================================
    return result
...
def is_no(one_more_input):
    ...
    result = any([one_more_input.lower()== 'n', one_more_input.lower()== 'no'])
    # ==================================
    return result
...
def main():
    ...
    while True:
        temp_input = input("Input guess number : ")
        if temp_input == '0':
            break

        if is_validated_number(temp_input):
            rst = get_strikes_or_ball(temp_input, random_number)
            if rst == [3, 0]:
                print("Strikes : {} , Balls : {}".format(rst[0], rst[1]))
                while True:
                    ques = input("You win, one more(Y/N)?")
                    if is_yes(ques):
                        k = 1
                        break
                    elif is_no(ques):
